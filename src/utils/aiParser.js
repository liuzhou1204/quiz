...
- multi（多选）：有多个正确选项，答案以逗号分隔如 "A,B,C"，或题干含"哪些/多项/以下正确的"或末尾有"（多选）""（多选题）"
- judge（判断）：答案为"正确"或"错误"，或"对/错"、"√/×"
- fill（填空）：题干含 ____ 或 （ ），答案为填空关键词

## 无题号题目识别（非常重要！）
很多文档的题目没有显式编号（如"1."、"2."），而是靠段落空白分隔。识别方法：
1. 将文档按空行分段，每个独立段落块可能包含一道题目
2. 题目块结构通常为：题干行(含嵌入式答案) → 选项行(A.B.C.D) → 解析行
3. 题干末尾如包含嵌入式答案如"（C）""（B、C）""（正确）""（错误）"，务必提取出正确答案，并从输出的题干文本中移除这些答案标记
4. 题干末尾如有"（多选）"或"（多选题）"标记，将该题 type 设为 multi，并从题干中移除该标记
5. 对于判断题，题干末尾有"（正确）"或"（错误）"的，type 设为 judge，从题干移除后提取答案
6. 选项行通常以"A." "B." "A、" "B、" 或纯字母"A~D"开头，需要提取为结构化 options 数组
7. 紧跟选项行之后的段落通常是解析说明，放入 explain 字段

### 嵌入式答案提取示例
输入："小张和小李在资源使用上有分歧，属于（C）"
→ q: "小张和小李在资源使用上有分歧，属于"  answer: "C"

输入："以下关于时差说法正确的是（B、C）（多选）"
→ q: "以下关于时差说法正确的是"  answer: "B,C"  type: "multi"

输入："项目目标要符合SMART原则（正确）"
→ q: "项目目标要符合SMART原则"  answer: "正确"  type: "judge"
...
3. 保持题目原始顺序
4. 如果文档中没有题目，返回空数组 []
5. 题干中不要包含题号（如"1."）和嵌入式答案标记（如"（C）""（多选）"）
6. 标题行（如"一、单选题""三、判断题""四、计算"）不是题目，跳过
7. 计算题/问答题的答案可能较长（多行），请完整提取`
